...
timestep = 100
def parse_executable_with_error (workload, qubits, bs):

    file_name = "{}_{}_{}".format (workload, qubits, bs)
    default_file = "./workloads_digiq_final/{}".format (file_name)
    executable_file = "./workloads_digiq_final/{}_error".format (file_name)
    if True:
...
def error_stabilizer_simulation (workload, qubits, bs, args):
    ...
    lines = f.readlines ()
    print("./workloads_digiq_final/{}_error".format (file_name))
    ...
    for inst_ in lines:
        ...
        if any ([key_ in words[0] for key_ in ['ry']]):
...
@ray.remote
def error_stabilizer_simulation_ray (workload, qubits, bs, args):
    return error_stabilizer_simulation (workload, qubits, bs, args)


def simulation_ray (workload, qubits, bs, num_shots, args):
    if not ray.is_initialized():
        ray.init()
    
    sims = [error_stabilizer_simulation_ray.remote (workload, qubits, bs, args) for _ in range (num_shots)]
...
def arg_parse ():
    ...
    parser.add_argument ("--bs", "-b", help="# of different 1Q gate executable in a group", type=int, default=8)
    ...
    parser.add_argument ("--measure", "-m", help="Measurement error rate", type=float, default=0.0145)
...
if __name__ == "__main__":
    ...
    for workload in workloads:
        for qubits in qubit_lists:
            parse_executable_with_error (workload, qubits, bs)
            if multithreading:
                results_error = simulation_ray (workload, qubits, bs, num_shots, args)
            else:
                results_error = list ()
            ...
            nf = open ("esm_errors/{}_{}.json".format (workload, qubits), "w")
    ...
    for distance in range(3,25,2):
        try:
            ...
            path = "esm_{}.json".format(num_qb)
            filename = os.getcwd() + "/esm_errors/esm_{}.json".format(num_qb)
