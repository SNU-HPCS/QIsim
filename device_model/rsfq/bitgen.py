...
FLAGS = flags.FLAGS
flags.DEFINE_integer ("ly", 225, "Length of Y-gate bitstream")
flags.DEFINE_integer ("lz", 256, "Length of Z-gate bitstream")
...
def gen_bitgen ():
    ...
    output_, depth = gen_shift_register (input_df, y_length+z_length, 1)
    outputs_ = gen_dff_output (output_, z_length, depth)
...
def gen_dff_output (input, target_dffs, depth):
    global connection_df, breakdown_df, y_length, z_length
    global num_and, num_or, num_dff, num_not, num_split
    if target_dffs <= 0:
        return input
...
def main (argv):
    ...
    output_list = None
    output_list = gen_bitgen ()
    connect_outputs (output_list)
    
    update_breakdown ()
    template = "{}/bitgen_{}y_{}z_{}.csv"
    connection_df.to_csv(template.format(unit_csv_dir, y_length, z_length, "connection"), index = False)
    breakdown_df.to_csv(template.format(unit_csv_dir, y_length, z_length, "breakdown"), index = False)
