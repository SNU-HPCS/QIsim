...
FLAGS = flags.FLAGS
flags.DEFINE_integer ("l", 481, "Length of bitstream")
...
def init_setup ():
    global connection_df, breakdown_df, length
...
def gen_bitgenctrl ():
    ...
    outputs_ = list ()
    # select declare
    input_df = pd.DataFrame({"Type": "input_go", "Name": "go"}, index=[0])
    output_ = gen_shift_register (input_df, length, 1)
    return output_ # DFF output ports


def gen_shift_register (input, target_dffs, depth):
    global connection_df, breakdown_df, y_length, z_length
    global num_and, num_or, num_dff, num_not, num_split
    if target_dffs <= 0:
        return input
    input_list_ = list ()
    ...
    output_ = None
    select_splits = ceil(log(target_dffs,2))
    ...
    for dff_index_ in range (target_dffs):
        ...
        if "input" in input_["Type"].item ():
            ...
            dff0_ = {"Type": "ORT_RSFQ", \
                     ...
                     "A_name": input_["Name"].item (), \
                     "A_depth_split": 0, \
                     "B_type": input["Type"].item (), \
                     "B_name": input["Name"].item (), \
                     "B_depth_split": select_splits, \
                     }
            num_or += 1
...
def connect_outputs (output):
    global connection_df, breakdown_df, y_length, z_length
    global num_and, num_or, num_dff, num_not, num_split
    
    # insert output
    out_ = {"Type": "output_ctrl", \
            "Name": "ctrl", \
            "Depth": output["Depth"].item () + 1, \
            "A_type": output["Type"].item (), \
            "A_name": output["Name"].item (), \
            "A_depth_split": 0, \
          }
...
def main (argv):
    ...
    output = gen_bitgenctrl ()
    connect_outputs (output)
    
    update_breakdown ()
    template = "{}/bitgenctrl_{}_{}.csv"
    connection_df.to_csv(template.format(unit_csv_dir, length, "connection"), index = False)
    breakdown_df.to_csv(template.format(unit_csv_dir, length, "breakdown"), index = False)
